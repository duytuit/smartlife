...
namespace AuthServer.Controllers
{
    public class AccountController : Controller
    {
        ...
        public async Task<IActionResult> ResetPassword([FromBody]ResetPasswordViewModel obj)
        {
            ...
            var reset_token = _userManager.GeneratePasswordResetTokenAsync(user).Result;
            IdentityResult result = _userManager.ResetPasswordAsync
                      (user, reset_token, obj.Password).Result;
            if (result.Succeeded)
            {
                ResultVM result_ResultVM = new ResultVM() {
                    Status = Status.Success,
                    Message = "Thành Công",

                };

                return Ok(result_ResultVM);
            }
            else
            {
                ResultVM result_ResultVM = new ResultVM()
                ...
                return NotFound(result_ResultVM);
            }
        }
        [HttpPost]
        [Route("api/[controller]/[action]")]
        public async Task<IActionResult> ChangePassword([FromBody]ChangePasswordViewModel obj)
        {
            ...
            IdentityResult result = _userManager.ChangePasswordAsync
                      (user, obj.currentPassword, obj.Password).Result;
            if (result.Succeeded)
            {
                ResultVM result_ResultVM = new ResultVM()
                {
